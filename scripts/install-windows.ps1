...
foreach ($RelativePath in $RequiredPaths) {
    if (-not (Test-Path -LiteralPath (Join-Path $SourceRoot $RelativePath))) {
        Fail "the repository checkout is incomplete: $RelativePath is missing."
    }
}
...
foreach ($RuntimeFile in @("meter.py", "page.html", "token_meter_mcp.py", "README.md", "LICENSE")) {
    $SourceFile = Join-Path $SourceRoot $RuntimeFile
    if (Test-Path -LiteralPath $SourceFile -PathType Leaf) {
        Copy-Item -LiteralPath $SourceFile -Destination (Join-Path $StagingRoot $RuntimeFile) -Force
    }
}
foreach ($RuntimeDirectory in @("menubar", "scripts", "assets")) {
    $SourceDirectory = Join-Path $SourceRoot $RuntimeDirectory
    if (Test-Path -LiteralPath $SourceDirectory -PathType Container) {
        Copy-Directory $SourceDirectory (Join-Path $StagingRoot $RuntimeDirectory)
    }
}
